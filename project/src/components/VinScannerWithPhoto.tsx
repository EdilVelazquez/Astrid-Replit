...
export function VinScannerWithPhoto({ onVinDetected, currentVin }: VinScannerWithPhotoProps) {
  ...
  const processImage = async (file: File) => {
    ...
    try {
      ...
      if (extractedVin) {
        ...
        setError('No se pudo detectar un VIN válido en la imagen. Por favor, intenta con otra foto más clara.');
        setDetectedVin(null);
  ...
  const handleConfirmVin = () => {
    if (detectedVin && capturedPhoto) {
      onVinDetected(detectedVin, capturedPhoto);
      setDetectedVin(null);
      setCapturedPhoto(null);
      setIsScanning(false);
    }
    ...
    return (
      <div className="mt-3 p-4 border-2 border-dashed border-green-400 bg-gradient-to-br from-green-50 to-blue-50 rounded-lg">
        <div className="text-center mb-3">
          ...
          <p className="text-xs text-gray-600">Escanea el VIN con tu cámara para capturar foto y texto automáticamente</p>
        </div>
        <button
          type="button"
    ...
    <div className="mt-3 p-4 border-2 border-green-200 rounded-lg bg-green-50">
      ...
      {isProcessing && (
        <div className="flex flex-col items-center justify-center py-8">
          ...
          <p className="text-sm text-gray-600">Analizando imagen del VIN...</p>
        </div>
      )}
        ...
        <div className="space-y-3">
          ...
          <div className="p-3 bg-blue-50 border border-blue-300 rounded-lg">
            <p className="text-xs text-blue-800">
              Al confirmar, el VIN se rellenará automáticamente y la foto se guardará como evidencia
            </p>
          </div>
